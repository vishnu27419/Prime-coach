...
const Model: React.FC<Props> = ({
  ...
  const handleClickCallback = (
    muscle: string,
    handleMuscleClick?: (data: MuscleStats) => void
  ) => {
...
// this is my function this is for reminder
addMuclesToArray = async (exercise) => {
  const {
    id,
    name,
    muscle,
  ...
  console.log("muscle", muscle, "id", id, "name", name);
  // console.log("ID", id);
  const { selectedMuscles, bodyData } = this.state;

  selectedMuscles.map((item) => {
    return item.muscles.push(muscle);
  });
  // selectedMuscles.push({ muscles: [muscle] });
  await this.setState({ selectedMuscles });
  // console.log("selected muscles are ", this.state.selectedMuscles);

  bodyData.push({ id: "1", name: "banch press", muscles: muscle });
  await this.setState({ bodyData, id: "", name: "", muscles: "" });
  console.log("Second Array for MAP===>>", this.state.bodyData);
};
...
// SD WORKOUT
{
  /* <div className="row">
              <div className="col-md-12">
                {this.props.weekArray.map((item) => {
                  return (
                    <div key={item.id}>
                      {item.days.map((item) => {
                        console.log("SECOND MODAL", item);
                        return (
                          <div key={item.id}>
                                    ...
                                    <div className="panel">
                                      ...
                                      <div>
                                        <iframe
                                          ...
                                          width="100%"
                                          height="315"
                                          src="https://www.youtube.com/embed/joGopvahtxU"
                                          frameBorder="0"
